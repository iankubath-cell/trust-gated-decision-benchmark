...
Scenario:
    ...
    Track 1 (Prior-Blind): Uses only sensor likelihood ratio
    Track 2 (Prior-Informed): Incorporates map prior into likelihood
...
class SensorBenchmarkConfig:
    ...
    # `stakes = |log(alpha/beta)|` actually varies across trials instead
    # of collapsing to a constant when alpha == beta.
...
def sample_sensor_trial_config(bench_config: SensorBenchmarkConfig,
                                rng: np.random.Generator,
                                trial_idx: int) -> SensorTrialConfig:
    ...
    # instead of being pinned at |log(alpha/beta)| == 0 whenever alpha == beta.
...
def generate_sensor_readings(config: SensorTrialConfig,
...
def load_kitti_data(kitti_path: str, 
                    max_frames: int = 1000) -> Optional[pd.DataFrame]:
    """
    Attempt to load real KITTI odometry data.
    
    This looks for KITTI-style pose and detection files. If not found,
    ...
    Args:
        kitti_path: Root directory of KITTI dataset
        max_frames: Maximum frames to load
        
    Returns:
        DataFrame with columns: frame_id, obstacle_detected, lidar_conf, camera_conf
    ...
    # Look for detection files (KITTI format: txt files per frame)
    ...
    for i, fname in enumerate(det_files[:max_frames]):
        ...
        try:
            # Parse KITTI detection file
            # Format: class truncated occluded alpha bbox dimensions location rotation
            with open(fpath, 'r') as f:
                lines = f.readlines()
            
            obstacle_detected = 1 if len(lines) > 0 else 0
            
            # Compute confidence from number and size of detections
            n_objects = len(lines)
            lidar_conf = min(n_objects / 5.0, 1.0)  # Heuristic
            
            # Average detection confidence (if available)
            confs = []
            for line in lines:
                parts = line.strip().split()
                if len(parts) >= 1:
                    confs.append(1.0)  # KITTI doesn't have explicit confidence
            
            camera_conf = min(np.mean(confs) if confs else 0.5, 1.0)
...
# =============================================================================
# SPRT FOR SENSOR FUSION
# =============================================================================

def calculate_sensor_log_lr(lidar_reading: int,
                              camera_reading: int,
                              p_h_lidar: float,
                              p_not_h_lidar: float,
                              p_h_camera: float,
                              p_not_h_camera: float) -> float:
    """
    Combined log-likelihood ratio from LiDAR + Camera readings.
    
    Assuming conditional independence given H:
    LLR = log P(lidar, camera | H) - log P(lidar, camera | ¬H)
        = log P(lidar | H) + log P(camera | H) - log P(lidar | ¬H) - log P(camera | ¬H)
    """
    def safe_llr(reading, p_h, p_not_h):
        p_h = max(p_h, 1e-10)
    ...
    llr_lidar = safe_llr(lidar_reading, p_h_lidar, p_not_h_lidar)
    llr_camera = safe_llr(camera_reading, p_h_camera, p_not_h_camera)
    
    return llr_lidar + llr_camera


def run_sensor_sprt(lidar_readings: np.ndarray,
                     ...
                     prior_belief: float = 0.5) -> Tuple[str, int, float]:
    """
    SPRT on fused sensor data with optional prior influence.
    
    Args:
        lidar_readings: Array of LiDAR detections (0/1)
        camera_readings: Array of Camera detections (0/1)
        p_h_lidar: P(detect | H, lidar)
        p_not_h_lidar: P(detect | ¬H, lidar)
        p_h_camera: P(detect | H, camera)
        p_not_h_camera: P(detect | ¬H, camera)
        upper: SPRT upper boundary
        lower: SPRT lower boundary
        max_steps: Maximum readings
        prior_weight: 0 = blind, 1 = fully informed
        prior_belief: Map's belief that obstacle exists
        
    Returns:
        Tuple of (decision, steps_taken, final_score)
    """
    # Adjust sensor likelihoods with prior
    p_h_lidar_eff = (1 - prior_weight) * p_h_lidar + prior_weight * prior_belief
    p_not_h_lidar_eff = (1 - prior_weight) * p_not_h_lidar + prior_weight * (1 - prior_belief)
    
    p_h_camera_eff = (1 - prior_weight) * p_h_camera + prior_weight * prior_belief
    p_not_h_camera_eff = (1 - prior_weight) * p_not_h_camera + prior_weight * (1 - prior_belief)
    
    # Clip to avoid degeneracy
    p_h_lidar_eff = np.clip(p_h_lidar_eff, 0.01, 0.99)
    p_not_h_lidar_eff = np.clip(p_not_h_lidar_eff, 0.01, 0.99)
    p_h_camera_eff = np.clip(p_h_camera_eff, 0.01, 0.99)
    p_not_h_camera_eff = np.clip(p_not_h_camera_eff, 0.01, 0.99)
    
    log_lr = 0.0
    for i in range(min(max_steps, len(lidar_readings))):
        llr = calculate_sensor_log_lr(
            lidar_readings[i], camera_readings[i],
            p_h_lidar_eff, p_not_h_lidar_eff,
            p_h_camera_eff, p_not_h_camera_eff
        )
        log_lr += llr
...
# =============================================================================
# GATING (Reuses gating.g_function)
# =============================================================================

def compute_divergence_sensor(prior_belief: float, 
                                sensor_mle: float) -> float:
...
def gate_logistic_sensor(divergence: float, stakes: float,
...
def run_single_sensor_trial(config: SensorTrialConfig,
                              bench_config: SensorBenchmarkConfig) -> Dict:
    """Run one complete sensor fusion trial with dual-track SPRT + trust gating."""
    rng = np.random.default_rng(seed=config.random_seed)
    
    # Generate sensor readings
    lidar_readings, camera_readings = generate_sensor_readings(config, bench_config, rng)
    ...
    # Sensor likelihoods under H and ¬H
...
def run_sensor_benchmark(bench_config: SensorBenchmarkConfig = None) -> pd.DataFrame:
    ...
    if kitti_df is not None:
        ...
        for i in range(n_trials):
            ...
            # Create trial config from KITTI data
    ...
    print(f"\nClass balance:")
    print(f"  Obstacle present: {df['h_is_true'].sum()} ({df['h_is_true'].mean():.1%})")
    print(f"  No obstacle:      {(1-df['h_is_true']).sum()} ({(1-df['h_is_true']).mean():.1%})")
    
    # Prior quality breakdown
    if 'prior_quality' in df.columns:
        print(f"\nPrior quality distribution:")
        for pq in ['aligned', 'stale', 'adversarial']:
            count = (df['prior_quality'] == pq).sum()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Domain B: Sensor Fusion Benchmark (TGAI)"
    )
    parser.add_argument("--n-trials", type=int, default=1000,
                        help="Number of trials")
    parser.add_argument("--readings-per-trial", type=int, default=50,
                        help="Sensor readings per trial")
    parser.add_argument("--obstacle-prob", type=float, default=0.55,
                        help="Probability of obstacle per trial")
    parser.add_argument("--aligned-prob", type=float, default=0.5,
                        help="Probability of aligned prior")
    parser.add_argument("--stale-prob", type=float, default=0.3,
                        help="Probability of stale prior")
    ...
    parser.add_argument("--alpha-range", type=float, nargs=2, default=(0.01, 0.1),
                        metavar=("LOW", "HIGH"),
                        help="Range to sample per-trial SPRT alpha (Type I error) from")
    parser.add_argument("--beta-range", type=float, nargs=2, default=(0.01, 0.1),
                        metavar=("LOW", "HIGH"),
                        help="Range to sample per-trial SPRT beta (Type II error) from")
    parser.add_argument("--data-mode", type=str, default="synthetic",
    ...
    parser.add_argument("--output", type=str, 
                        default="results/domain_b_data.csv",
    ...
    if abs(total - 1.0) > 0.01:
        raise ValueError(
            f"Prior quality probabilities must sum to 1.0, got {total:.3f}"
        )
    ...
    # Patch #2: os.path.dirname("") is "" when --output has no directory
    # component, and os.makedirs("") raises FileNotFoundError — only
    # create the directory if there actually is one.
